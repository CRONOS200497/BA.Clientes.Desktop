...
{
    public partial class AgregarAudienciaProcuraduria : Form
    {
        public int Id_Perfil;
        public string Usuario;
        public int id_Audiencia;

        public AgregarAudienciaProcuraduria(int Id_Perfil, string User, int id_Audiencia)
        {
            if (id_Audiencia == 0)
            {
                InitializeComponent();
        ...
        AudienciasProcuraduriasController Audiencias = new AudienciasProcuraduriasController();
        ClientesController Negocio = new ClientesController();
        FamiliaController Familiar = new FamiliaController();
        ProcuraduriasController Procuraduria = new ProcuraduriasController();
        ...
        AudienciasProcuraduriaViews EntidadJuzgados = new AudienciasProcuraduriaViews();
        string Usuario_Login = string.Empty;
        int PerfilRol = 0;
        int numero_archivo = 0;
        int value_Procuraduria = 0;
        ...
        public void ConsultarAudiencia(int id_Audiencia)
        {
            ...
            {
                numero_audiencia = datos_audiencia.id_Audiencia_Procuraduria;
                ...
                Nombre_Procuraduria = datos_audiencia.Procuraduria;
                CargarJuzgadosPorNombre(Nombre_Procuraduria);
                ...
                cbModificarProcuraduria.Enabled = true;
        ...
        {
            var Load_Mostrar_Juzgados = Procuraduria.MostrarProcuradurias();
            if (Load_Mostrar_Juzgados.Count() > 0)
            {
                cbProcuraduria.DisplayMember = "Nombre_Procuraduria";
                cbProcuraduria.ValueMember = "id_Procuraduria";
                cbProcuraduria.DropDownStyle = ComboBoxStyle.DropDownList;
                cbProcuraduria.DataSource = Load_Mostrar_Juzgados;
            }
            else
            {
                cbProcuraduria = null;
        ...
        private void CargarJuzgadosPorNombre(string Nombre_Procuraduria)
        {
            var Load_Mostrar_Juzgados = Procuraduria.MostrarJuzgadoSeleccionado(Nombre_Procuraduria);
            cbProcuraduria.DisplayMember = "Nombre_Procuraduria";
            cbProcuraduria.ValueMember = "id_Procuraduria";
            cbProcuraduria.DropDownStyle = ComboBoxStyle.DropDownList;
            cbProcuraduria.DataSource = Load_Mostrar_Juzgados;
        ...
        private void btnAdjuntarDocumento_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                string documentos = "Audiencia_Procuraduria";
        ...
        public void Guardar()
        {
            string Ruta_Original = Parametros.MostrarParametro("RutaAudienciasProcuraduria");
            ...
            string mes = DateTime.Now.Month.ToString();
            string dia = DateTime.Now.Day.ToString();
            documentos = "Audiencia_Procuraduria";
            ...
            EntidadJuzgados.Procuraduria = Nombre_Procuraduria;
            ...
            if (validar_guardado == true)
            {
                ...
                MessageBox.Show("La audiencia de procuraduria ha sido registrada correctamente.");
                this.Close();
            }
            else
            {
                MessageBox.Show("La audiencia de procuraduria no pudo ser registrada, por favor verifique la información y oprima guardar");
            }
        }

        private void cbJuzgado_SelectedValueChanged(object sender, EventArgs e)
        {
            value_Procuraduria = Convert.ToInt32(cbProcuraduria.SelectedValue);
            Nombre_Procuraduria = Convert.ToString(Procuraduria.MostrarProcuraduriasNombre(value_Procuraduria));
        }
        ...
        public void Modificar()
        {
            ...
            EntidadJuzgados.id_Audiencia_Procuraduria = numero_audiencia;
            EntidadJuzgados.Doc_Cliente = txtNumeroIdentificacionCliente.Text;
            EntidadJuzgados.Primer_Nombre = txtPrimerNombreCliente.Text;
            EntidadJuzgados.Segundo_Nombre = txtSegundoNombreCliente.Text;
            EntidadJuzgados.Primer_Apellido = txtPrimerApellidoCliente.Text;
            ...
            if (validar_guardado == true)
            {
                ...
                lblModificado.Text = EntidadJuzgados.Fecha_Actualizado.ToString();
                lblSubidoPor.Text = EntidadJuzgados.Usuario;
                MessageBox.Show("La audiencia de procuraduria ha sido modificado correctamente.");
                this.Close();
            }
            else
            {
                MessageBox.Show("La audiencia de procuraduria no pudo ser modificada, por favor verifique la información y oprima modificar.");
            }
        }
        ...
        {
            DialogResult ModificarProcuraduria = MessageBox.Show("Confirme si desea modificar la procuraduria.", "¡Alerta!", MessageBoxButtons.YesNo);
            if (ModificarProcuraduria == DialogResult.Yes)
            {
                CargarJuzgados();
            }
            else if (ModificarProcuraduria == DialogResult.No)
            {
                cbModificarProcuraduria.Checked = false;
                return;
            }
            else
            {
