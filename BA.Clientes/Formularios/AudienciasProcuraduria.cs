...
{
    public partial class AudienciasProcuraduria : Form
    {
        public int Id_Perfil;
        public string Usuario;

        public AudienciasProcuraduria(int Id_Perfil, string User)
        {
            Usuario_Login = User;
            PerfilRol = Id_Perfil;
            InitializeComponent();
        ...
        private void btnAudienciasMes_Click(object sender, EventArgs e)
        {
            ...
            {
                dataGridView1.Rows.Add(d.id_Audiencia_Procuraduria, d.Doc_Cliente, d.Primer_Nombre, d.Segundo_Nombre, d.Primer_Apellido, d.SegundoApellido, d.Numero_Proceso, d.Procuraduria, d.Fecha_Audiencia, d.Abogado, d.Correo_Abogado, d.Observaciones, d.Adjuntar_Archivo, d.Informar_Cliente, d.Informar_Abogado, d.Fecha_Subido, d.Fecha_Actualizado, d.Usuario);
            }
        ...
        {
            string Nombre_Formulario = "GuardarAudienciasProcuraduria";
            ...
            if (Roles.Count > 0)
            {
                foreach (var d in Roles)
                {
                    ...
                    {
                        AgregarAudienciaProcuraduria audienciaProcuraduria = new AgregarAudienciaProcuraduria(Id_Perfil, Usuario_Login, 0);
        ...
        {
            string Nombre_Formulario = "ModificarAudienciasProcuraduria";
            var Roles = Rol.ConsultarRolesPorPerfil(Id_Perfil, Nombre_Formulario);
            if (Roles.Count > 0)
            {
                foreach (var d in Roles)
                {
                    if (d.Nombre_Rol.Equals(Nombre_Formulario))
                    {
                        AgregarAudienciaProcuraduria audienciaProcuraduria = new AgregarAudienciaProcuraduria(Id_Perfil, Usuario_Login, id_Audiencia);
                        audienciaProcuraduria.ShowDialog();
                    }
                    else
                    {
        ...
        private void dataGridView1_CellContentClick_1(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                {
                    string Nombre_Formulario = "VerDocumentosAudienciasProcuraduria";
                    ...
                    if (Roles.Count > 0)
                    {
                        foreach (var d in Roles)
                        {
                            if (d.Nombre_Rol.Equals(Nombre_Formulario))
                            {
                                ...
                                if (Ruta.Length > 0)
                                {
                                    VerDocumentosAudienciasProcuraduria ver = new VerDocumentosAudienciasProcuraduria(id_Archivo, PerfilRol, Usuario_Login);
                                    ver.Show();
                                    return;
                                }
                                else
                                {
                                    MessageBox.Show("La audiencia de procuraduria seleccionada no tiene adjunto.");
                                }
                            }
                            else
                            {
        ...
        {
            string Nombre_Formulario = "EliminarAudienciasProcuraduria";
            var Roles = Rol.ConsultarRolesPorPerfil(Id_Perfil, Nombre_Formulario);
            if (Roles.Count > 0)
            {
                foreach (var d in Roles)
                {
                    if (d.Nombre_Rol.Equals(Nombre_Formulario))
                    {
                        ...
                        EntidadJuzgados.id_Audiencia_Procuraduria = id_Audiencia;
